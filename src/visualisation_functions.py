...
GUYS_CODES = [11842, 12230, 12455, 15344, 33717, 93898]
...
def create_parameters_table(data):
    ...
    max_possible_priority = MergedReq['Preority_weight'].sum()
...
def coloured_table(tab, name, colours_from=MAIN_COLOUR, colours_to='rgb(255, 255, 255)'):
...
def get_priority_graph(data, name, bins_num=HEADER_FONT_SIZE):
    """Функция, выдающая диаграммы приоритетов

    Args:
        data (input_file): [description]
        name (str): [description]
        bins_num (int, optional): [description]. Defaults to HEADER_FONT_SIZE.
    """
    MergedReq = data.get_MergedReq()
    ...
    fig = go.Figure(data=[                       
        go.Histogram(
                                ...
                                layout=dict(title=dict(text=name,
                                                    font = dict(color=MAIN_COLOUR, size=BIG_HEADER_FONT_SIZE)),
                                                    barmode='group',
                                                    plot_bgcolor='rgb(255,255,255)',
height=500 ,                                          xaxis = dict(
        color = MAIN_COLOUR),
...
def histogram_working_hours(data, name, bins_num=20):
...
def get_gantt_for_one_data(rest_table_merged, guy=11835):
    ...
    merged["date_start"] = merged.apply(lambda x: calculate_date_for_gantt(x['month'], x['MaxFly'], x["iRestHoursP1"], True), axis=1)
    merged["date_finish"] = merged.apply(lambda x: calculate_date_for_gantt(cycled_indexes2[x['month']], x['MaxFly'], x["iRestHoursP2"], False), axis=1)
...
def get_gantt_fig(data, name, guys):
    ...
    for guy in guys:
        merged = merged.append(get_gantt_for_one_data(rest_table_merged, guy), ignore_index=True)

    fig = px.timeline(merged, x_start="date_start", x_end="date_finish", y="worker", color = "worker", color_discrete_map = {str(guy): MAIN_COLOUR for guy in guys}, range_x=[datetime.date(YEAR, 1, 1), datetime.date(YEAR, 12, 31)])


    fig.update_layout(dict(title=dict(text=name,
                                                    font = dict(color=MAIN_COLOUR, size=BIG_HEADER_FONT_SIZE)),
                                                        xaxis = dict(
        tickmode = 'linear',
        dtick = 'M1',
...
right_panel = html.Div(className='main',  # Define the row element
                        children = [html.H1('Решение команды NO CONSTRAINTS', style = dict(textAlign = 'center', color=MAIN_COLOUR, size=BIG_HEADER_FONT_SIZE + 40)),
                        dcc.Graph(id='Целевые показатели',
                                                config={'displayModeBar': False},
                                                animate=True,
                                                figure=create_parameters_table(data)),
                                    
                                            dcc.Graph(id='Количество рабочих часов',
                                                config={'displayModeBar': False},
                                                animate=True,
                                                figure=coloured_table(data.get_working_hours(), 'Количество рабочих часов')),

                                    dcc.Graph(id='Дефицит ресурсов',
                                                config={'displayModeBar': False},
                                                animate=True,
                                                figure=coloured_table(data.get_qual_deficit(), 'Дефицит ресурсов')),

                                    dcc.Graph(id='Распределение часов отпуска',
                                                config={'displayModeBar': False},
                                                animate=True,
                                                figure=histogram_working_hours(data, 'Распределение часов отпуска')),
                
                                    dcc.Graph(id='Гистограммы приоритета',
                                                config={'displayModeBar': False},
                                                animate=True,
                                                figure=get_priority_graph(data, "Распределение приоритетов")),

                                    dcc.Graph(id='Гант чарт',
                                                config={'displayModeBar': False},
                                                animate=True,
                                                figure=get_gantt_fig(data, "Примеры графиков", GUYS_CODES))
                            ])

app_children = html.Div(className='row',  # Define the row element
